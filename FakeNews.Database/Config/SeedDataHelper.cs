...
namespace FakeNews.Database.Config
{
    ...
    {
        internal void FeedSeedData(ApplicationDbContext dbContext)
        {
            if (IsDataSeeded(dbContext) is false)
            {
                GenerateFirstData(dbContext);
            }
        }

        private bool IsDataSeeded(ApplicationDbContext dbContext)
        {
            var hasData = dbContext.Users.Any();

            if (hasData is false)
                hasData = dbContext.Roles.Any();

            if (hasData is false)
                hasData = dbContext.News.Any();

            if (hasData is false)
                hasData = dbContext.Categories.Any();

            if (hasData is false)
                hasData = dbContext.Comments.Any();

            if (hasData is false)
                hasData = dbContext.NewsUserSeens.Any();

            return hasData;
        }

        private void GenerateFirstData(ApplicationDbContext dbContext)
        {
            ...
            //اخبار و مقالات
            dbContext.News.Add(new Tables.News()
            {
                AuthorId = 1,
                CreatedOn = System.DateTime.Now,
                CreatorId = 1,
